    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Geri Bildirim Bırak</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div>
            <h3 className="font-medium text-gray-900 mb-2">
              {appointment.teacherName} ile Seans
            </h3>
            <p className="text-sm text-gray-600">
              {new Date(appointment.appointmentTime).toLocaleDateString('tr-TR')} saat {' '}
              {new Date(appointment.appointmentTime).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}
            </p>
            {appointment.subject && (
              <p className="text-sm text-gray-600">Konu: {appointment.subject}</p>
            )}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-3">
              Bu seansı nasıl değerlendirirsiniz?
            </label>
          ...
          <div>
            <label htmlFor="comment" className="block text-sm font-medium text-gray-700 mb-2">
              Deneyiminizi paylaşın (isteğe bağlı)
            </label>
            <textarea
              id="comment"
              value={comment}
              ...
              placeholder="Seansınız nasıldı? Neler öğrendiniz? Önerileriniz var mı?"
            />
          </div>
